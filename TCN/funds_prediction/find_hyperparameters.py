...
from TCN.my_tcn.model import TCN
from TCN.my_tcn.utils import *
import pandas as pd
...
def objective(trail:optuna.Trial):
    seq_length = trail.suggest_int('seq_length',5,14)
    print("Produce data...")
    x_train, y_train, x_test, y_test, y_mean, y_std, x_predict = generate_data()
    ...
    predict_x = generate_predict_samples(x_predict, seq_length)
    # 输入维度和输出维度
